'use client'

import { useEffect, useState } from 'react'
import dynamic from 'next/dynamic'
import { supabase, Shop } from '@/lib/supabase'
import { MapPin, Heart, Search, Coffee, Utensils, Beer, Pizza, IceCream, Store, ChevronRight } from 'lucide-react'

const ShopMap = dynamic(() => import('@/components/ShopMap'), {
  ssr: false,
  loading: () => <div className="w-full h-full bg-gray-100 flex items-center justify-center font-bold text-gray-400">地図を読み込み中...</div>,
})

// カテゴリーとアイコンの定義
const CATEGORIES = [
  { id: 'カフェ', name: 'カフェ', icon: <Coffee size={14} />, color: 'bg-orange-100 text-orange-600' },
  { id: '居酒屋', name: '居酒屋', icon: <Beer size={14} />, color: 'bg-yellow-100 text-yellow-600' },
  { id: 'イタリアン', name: 'パスタ', icon: <Pizza size={14} />, color: 'bg-red-100 text-red-600' },
  { id: '焼肉', name: '焼肉', icon: <Utensils size={14} />, color: 'bg-rose-100 text-rose-600' },
  { id: 'スイーツ', name: 'スイーツ', icon: <IceCream size={14} />, color: 'bg-pink-100 text-pink-600' },
  { id: '和食', name: '和食', icon: <Store size={14} />, color: 'bg-emerald-100 text-emerald-600' },
]

export default function Taberu() {
  const [allShops, setAllShops] = useState<Shop[]>([])
  const [filteredShops, setFilteredShops] = useState<Shop[]>([])
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null)
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    async function fetchShops() {
      try {
        setLoading(true)
        const { data, error } = await supabase.from('shops').select('*')
        if (error) throw error
        const foodOnly = (data || []).filter(s => s.category !== '買い物')
        setAllShops(foodOnly)
        setFilteredShops(foodOnly)
      } catch (err) {
        console.error(err)
      } finally {
        setLoading(false)
      }
    }
    fetchShops()
  }, [])

  useEffect(() => {
    let result = allShops
    if (selectedCategory) {
      result = allShops.filter(s => s.category === selectedCategory)
    }
    setFilteredShops(result)
  }, [selectedCategory, allShops])

  if (loading) return <div className="p-10 text-center font-bold text-blue-600">読み込み中...</div>

  return (
    <div className="relative h-screen w-full bg-white overflow-hidden">
      
      {/* 1. 地図レイヤー */}
      <div className="absolute inset-0 z-0 pointer-events-auto">
        <ShopMap shops={filteredShops} />
      </div>

      {/* 2. 上部UI：検索と極小カテゴリー（横スライド） */}
      <div className="absolute inset-0 z-10 pointer-events-none">
        <div className="mt-4 px-4 space-y-3">
          {/* 検索バー */}
          <div className="bg-white shadow-lg rounded-full flex items-center p-3 px-4 gap-3 pointer-events-auto border border-gray-100 max-w-md mx-auto">
            <Search size={16} className="text-gray-400" />
            <input 
              type="text" 
              placeholder="お店を検索" 
              className="text-xs font-bold text-gray-800 outline-none w-full bg-transparent"
            />
          </div>
          
          {/* 横スライド・極小カテゴリーボタン */}
          <div className="flex overflow-x-auto no-scrollbar gap-2 pointer-events-auto pb-2">
            {CATEGORIES.map((cat) => (
              <button
                key={cat.id}
                onClick={() => setSelectedCategory(selectedCategory === cat.id ? null : cat.id)}
                className={`flex items-center gap-1.5 p-1 pr-3 rounded-full transition-all shrink-0 border ${
                  selectedCategory === cat.id 
                    ? 'bg-orange-500 border-orange-600 text-white shadow-md' 
                    : 'bg-white/95 border-gray-100 text-gray-700 shadow-sm'
                }`}
              >
                <div className={`w-7 h-7 rounded-full flex items-center justify-center ${selectedCategory === cat.id ? 'bg-white/20 text-white' : cat.color}`}>
                  {cat.icon}
                </div>
                <span className="text-[10px] font-black whitespace-nowrap">{cat.name}</span>
              </button>
            ))}
          </div>
        </div>
      </div>

      {/* 3. 下部リストレイヤー */}
      <div className="absolute inset-0 z-20 pointer-events-none overflow-y-auto no-scrollbar">
        {/* mt-[50vh] に戻して、地図が見える面積を確保 */}
        <div className="mt-[50vh] min-h-screen bg-white rounded-t-[2rem] shadow-[0_-10px_40px_rgba(0,0,0,0.15)] pointer-events-auto pb-32 border-t border-gray-100">
          <div className="w-10 h-1 bg-gray-200 rounded-full mx-auto relative top-3" />
          
          <div className="p-6 pt-10">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-black text-gray-900">
                {selectedCategory ? `${selectedCategory}` : '近くの人気店'}
                <span className="text-xs font-bold text-gray-400 ml-2">{filteredShops.length}件</span>
              </h2>
            </div>

            <div className="space-y-8">
              {filteredShops.map((shop) => (
                <div key={shop.id} className="group pb-6 border-b border-gray-50">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-md font-black text-gray-900 leading-tight">{shop.name}</h3>
                    <Heart size={18} className="text-gray-300" />
                  </div>
                  <p className="text-[10px] text-gray-500 mb-3 flex items-center gap-1">
                    <MapPin size={10} className="text-orange-500" />
                    {shop.address}
                  </p>
                  {/* 小さめの画像プレースホルダー */}
                  <div className="grid grid-cols-3 gap-1.5 rounded-xl overflow-hidden h-20 bg-gray-50">
                    <div className="bg-gray-100" />
                    <div className="bg-gray-100" />
                    <div className="bg-gray-100" />
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>

      <style jsx global>{`
        .no-scrollbar::-webkit-scrollbar { display: none; }
        .no-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
      `}</style>
    </div>
  )
}