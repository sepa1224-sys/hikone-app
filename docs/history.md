...
2026-02-13 (Continued): 学生コミュニティへのUI導線実装

**概要**:
ホーム画面（`app/page.tsx`）に表示されている学生所属ラベルをクリッカブルなリンクに変更し、新設した学校詳細ページ（`app/school/[id]/page.tsx`）への遷移を実装した。
これにより、学生ユーザーは自身の所属する学校コミュニティへスムーズにアクセスできるようになった。

**変更点**:
- **UI実装**:
    - `app/page.tsx`: 所属表示エリアを `next/link` でラップし、ホバー効果と `ChevronRight` アイコンを追加してタップ可能性を視覚的に表現。
    - `app/school/[id]/page.tsx`: 学校詳細ページのスケルトンを作成。基本情報、人数統計、ランキングのプレースホルダーを配置。
- **仕様書更新**:
    - `docs/specs/student-community.md`: ホーム画面からの遷移フローとUI仕様を追記。
